...
public class OmnipodPdm {
        ...
        if (result != null && !result.canceled && result.status != null) {
                    ...
                    _lastStatus.id_lot != result.status.id_lot || _lastStatus.id_t != result.status.id_t))
            {
                ...
                Notification notification = new Notification(Notification.OMNIPY_POD_CHANGE,
                        String.format("@string/omnipod_pod_state_POD_IDs_has_been_removed", _lastStatus.id_lot, _lastStatus.id_t), Notification.NORMAL);       //"Pod with Lot %d and Serial %d has been removed."
                _lastStatus = null;
            ...
            if (result.status.state_progress == 0 && (_lastStatus == null || _lastStatus.state_progress != 0))
            {
                ...
                Notification notification = new Notification(Notification.OMNIPY_POD_STATUS, "@string/omnipod_pod_state_No_pod_registered", Notification.NORMAL); //"No pod registered"
                MainApp.bus().post(new EventNewNotification(notification));
            }
            ...
            {
                MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_POD_STATUS));
                Notification notification = new Notification(Notification.OMNIPY_POD_STATUS, "@string/omnipod_pod_state_Pod_in_activation_progress", Notification.INFO);      //"Pod in activation progress"
                MainApp.bus().post(new EventNewNotification(notification));
            }
            else if (result.status.state_progress == 8 && (_lastStatus == null || _lastStatus.state_progress < 8))
            {
                ...
                MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_POD_STATUS));
                Notification notification = new Notification(Notification.OMNIPY_POD_STATUS, "@string/omnipod_pod_state_Pod_is_activated_and_running", Notification.INFO);      //"Pod is activated and running"
                MainApp.bus().post(new EventNewNotification(notification));
            }
            else if (result.status.state_progress == 9 && (_lastStatus == null || _lastStatus.state_progress == 8))
            {
                ...
                MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_POD_STATUS));
                Notification notification = new Notification(Notification.OMNIPY_POD_STATUS, "@string/omnipod_pod_state_Pod_running_with_low_reservoir_less_than_50U", Notification.NORMAL);    //"Pod running with low reservoir (less than 50U)"
                MainApp.bus().post(new EventNewNotification(notification));
            }
            else if (result.status.state_progress > 9 && (_lastStatus == null || _lastStatus.state_progress <= 9))
            {
                ...
                MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_POD_STATUS));
                Notification notification = new Notification(Notification.OMNIPY_POD_STATUS, "@string/omnipod_pod_state_Pod_stopped", Notification.NORMAL);       //"Pod stopped"
                MainApp.bus().post(new EventNewNotification(notification));
            }
                ...
                && (_lastStatus == null || _lastStatus.state_alert != result.status.state_alert))
            {
                ...
                MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_POD_STATUS));
                Notification notification = new Notification(Notification.OMNIPY_POD_STATUS, "@string/omnipod_pod_state_Pod_alert" + alertText, Notification.NORMAL);       //"Pod alert: "
                MainApp.bus().post(new EventNewNotification(notification));
            }
    ...
    public void onConfigurationComplete(final EventOmnipyConfigurationComplete confResult) {
        ...
        if (!confResult.isConnectable)
        {
            ...
            {
                errorMessage = "@string/omnipod_network_status_located_no_connection_txt1" + confResult.hostName +         //"Omnipy located at network address "
                        "@string/omnipod_network_status_located_no_connection_txt2";                                    //" but connection cannot be established"
            }
            else
            {
                errorMessage = "@string/omnipod_network_status_not_located_no_connection_txt1" + confResult.hostName      //"Omnipy connection cannot be established at the configured address: "
                        + "@string/omnipod_network_status_not_located_no_connection_txt2";                                                       //" Please verify the address in configuration."
            }
            MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_CONNECTION_STATUS));
        ...
        {
            errorMessage = "@string/omnipod_network_status_located_no_authentication_txt1" + confResult.hostName +                          //"Omnipy connection established at address "
                    "@string/omnipod_network_status_located_no_authentication_txt2";                             //" but authentication failed. Please verify your password in settings."
            MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_CONNECTION_STATUS));
            Notification notification = new Notification(Notification.OMNIPY_CONNECTION_STATUS, errorMessage, Notification.NORMAL);
            MainApp.bus().post(new EventNewNotification(notification));
        ...
        else
        {
            ...
            Notification notification = new Notification(Notification.OMNIPY_CONNECTION_STATUS,
                    "@string/omnipod_network_status_located_and_connected" + confResult.hostName, Notification.INFO, 1);             //"Connected to omnipy running at "
            MainApp.bus().post(new EventNewNotification(notification));
    ...
    public ArrayList<String> getAlerts(int alertMask)
    {
        ...
        if ((alertMask & 0x01) > 0)MainApp.bus().post(new EventDismissNotification(Notification.OMNIPY_CONNECTION_STATUS));
            alerts.add("@string/omnipod_pod_alerts_Auto_off");     //"Auto-off"
        if ((alertMask & 0x02) > 0)
            alerts.add("@string/omnipod_pod_alerts_Unknown");      //"Unknown"
        if ((alertMask & 0x04) > 0)
            alerts.add("@string/omnipod_pod_alerts_Pod_expiring_soon");        //"Pod expiring soon"
        if ((alertMask & 0x08) > 0)
            alerts.add("@string/omnipod_pod_alerts_Replace_pod_soon");     //"Replace pod soon"
        if ((alertMask & 0x10) > 0)
            alerts.add("@string/omnipod_pod_alerts_Low_reservoir");        //"Low reservoir"
        if ((alertMask & 0x20) > 0)
            alerts.add("@string/omnipod_pod_alerts_Insulin_Suspended");        //"Insulin Suspended"
        if ((alertMask & 0x40) > 0)
            alerts.add("@string/omnipod_pod_alerts_End_of_insulin_suspend");       //"End of insulin suspend"
        if ((alertMask & 0x80) > 0)
            alerts.add("@string/omnipod_pod_alerts_Pod expired");      //"Pod expired"
        return alerts;
    }
    ...
    {
        if (_lastStatus == null)
            return "@string/omnipod_pod_status_Status_unknown";        //"Status unknown"

        StringBuilder sb = new StringBuilder();
        sb.append("Lot: ").append(_lastStatus.id_lot).append(" TID: ").append(_lastStatus.id_t)
                .append("@string/omnipod_pod_status_radio_address") //" Radio address: "
                .append(String.format("%08X", _lastStatus.radio_address));

        sb.append("@string/omnipod_pod_status_status");
        if (_lastStatus.state_progress < 8)
        {
            sb.append("@string/omnipod_pod_status_Not_yet_running");    //"Not yet running"
        }
        else if (_lastStatus.state_progress == 8)
        {
            sb.append("@string/omnipod_pod_status_Running");   //"Running"
        }
        else if (_lastStatus.state_progress == 9)
        {
            sb.append("@string/omnipod_pod_status_Running_with_low_insulin_below50U");   //"Running with low insulin (<50U)"
        }
        else
        {
            sb.append("@string/omnipod_pod_status_Inactive");  //"Inactive"
        }

        if (_lastStatus.state_faulted)
        {
            sb.append("@string/omnipod_pod_status_POD_FAULTED");     //"\nPOD FAULTED"
        }
        ...
        sb.append("@string/omnipod_pod_status_Total_insulin_delivered").append(String.format("%3.2f",      //"\nTotal insulin delivered: "
                _lastStatus.insulin_delivered)).append("U");
        sb.append("@string/omnipod_pod_status_Reservoir");     //"\nReservoir: "
        if (_lastStatus.insulin_reservoir > 50)
            sb.append("@string/omnipod_pod_status_more_than_50U");      //"more than 50U"
        else
            sb.append(String.format("%2.2f", _lastStatus.insulin_reservoir)).append("U");

        sb.append("@string/omnipod_pod_status_Pod_age").append(String.format("%dd%dh%dm", days, hours, minutes));       //"\nPod age: "
        ...
        sb.append("@string/omnipod_pod_status_Status_updated").append(dateFormat.format(date));        //"\nStatus updated: "
    ...
    public String getConnectionStatusText()
    {
        ...
        if (!_restApi.isConfigured())
        {
            sb.append("@string/omnipod_pod_connection_status_Address_configuration_in_progress");      //"Address configuration in progress..."
        }
        else
        {
            if (_restApi.isDiscovered())
                sb.append("@string/omnipod_pod_connection_status_Discovered_Address").append(_restApi.getHost());       //"Discovered Address: "
            else
                sb.append("@string/omnipod_pod_connection_status_Configured_Address").append(_restApi.getHost());       //"Configured Address: "

            if (!_restApi.isConnectable())
                sb.append("@string/omnipod_pod_connection_status_Connectable_No");     //"\nConnectable: No"
            else
            {
                sb.append("@string/omnipod_pod_connection_status_Connectable_Yes");        //"\nConnectable: Yes"
                if (_restApi.isAuthenticated())
                    sb.append("@string/omnipod_pod_connection_status_Authentication_Verified");        //"\nAuthentication: Verified"
                else
                    sb.append("@string/omnipod_pod_connection_status_Authentication_Failed");      //"\nAuthentication: Failed"
                if (_lastResult != null && _lastResult.api != null)
                {
                    sb.append("@string/omnipod_pod_connection_status_Omnipy_API_Version" + _lastResult.api.version_major         //"\nOmnipy API Version: v"
                        + "." + _lastResult.api.version_minor);
                }
            }
        }

        sb.append("@string/omnipod_pod_connection_status_Last_successful_connection");        //"\nLast successful connection: "
        long tx = _restApi.getLastSuccessfulConnection();
        if (tx == 0)
        {
            sb.append("@string/omnipod_pod_connection_status_NA");   //"N/A"
        }
        else
        {
